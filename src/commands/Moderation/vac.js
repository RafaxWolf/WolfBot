...
async execute (client, message, args) {
    ...
    if(target.id === "765454346061086770") return message.author.send(`:x: | No puedes VAC Banear al bot!\n${message.author}`);
    if(target.id === message.author.id) return message.author.send(":x: | No puedes VAC Banear a ti mismo!");
